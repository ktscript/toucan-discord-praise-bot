...
export const SlackAuthBtn = ({ extraClasses }: ifcBtnProps) => {
  return (
    <button
      onClick={(e) => {
        e.preventDefault();
      ...
      className={`${extraClasses} inline-flex justify-center py-2 px-4 border border-gray-300 rounded-md shadow-sm bg-white text-sm font-medium text-gray-500 hover:opacity-70`}
    >
      <span className="mr-1">Sign in with Slack</span>
      <Image width={20} height={20} src="/slack-logo.svg" className="w-5 h-5" />
    </button>
  );
};
